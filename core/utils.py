...
def print_and_write(file_handle, text, c=37 , f=0, e='\n'):
    print_f(text, c, f, e)
...
def print_f(info, color=31, format=0, enter='\n'):
    # 格式：
    ...
    # 7                反白显示
    # 8                不可见
    print(f'\033[{format};{color}m{info}\033[0m', end=enter) if color or format else print(f'{info}', end=enter)


def get_object(depth_img, cam_intrinsics, cam_pose, pix):

    # Get 3D point cloud from RGB-D images (camera coordinates)
    ...
    # Zc*[U-U0]/Fx = Xc  像素坐标系转相机坐标系
    cam_pts_x = np.multiply(pix_x - cam_intrinsics[0][2], depth_img / cam_intrinsics[0][0])
    cam_pts_y = np.multiply(pix_y - cam_intrinsics[1][2], depth_img / cam_intrinsics[1][1])
    ...
    t_obj2cam = cam_pts[pix[0] * im_w + pix[1], :]
    print("T_obj2cam: {}".format(t_obj2cam))

    # Transform 3D point cloud from camera coordinates to robot coordinates
    # T_obj2base = T_cam2base + T_obj2cam
    surface_pts = np.transpose(np.dot(cam_pose[0:3, 0:3], np.transpose(cam_pts)) +
                               np.tile(cam_pose[0:3, 3:], (1, cam_pts.shape[0])))
    t_obj2base = surface_pts[pix[0] * im_w + pix[1], :]
    print("T_obj2base: {}".format(t_obj2base))

    # rob_pts_x = surface_pts[:, 0]
    # rob_pts_y = surface_pts[:, 1]
    # rob_pts_z = surface_pts[:, 2]
    # rob_pts_x.shape = (1, im_h, im_w)
    # rob_pts_y.shape = (1, im_h, im_w)
    # rob_pts_z.shape = (1, im_h, im_w)
    # rob_pts = np.concatenate((rob_pts_x, rob_pts_y, rob_pts_z), axis=0)

    return t_obj2cam, t_obj2base
...
def get_heightmap(color_img, depth_img, cam_intrinsics, cam_pose, workspace_limits, heightmap_resolution):
    ...
    # Filter out surface points outside heightmap boundaries (只保留大小限制在workspace_limits的坐标)
